...
const Cart = () => {
  ...
  useEffect(() => {
    setTotalAmount(cart.reduce((acc, curr) => acc + curr.price, 0)); // Adding the total amounts of items in the cart
  }, [cart]);
    ...
    <div>
      {cart.length > 0 ? (
        <div className="max-w-[1200px] mx-auto flex flex-col md:flex-row justify-center">
          <div className="w-full md:w-[75%] flex flex-col p-2">
            {cart.map((item, index) => {
              return (
                <CartItem
                  key={item.id}
          ...
          <div className="w-full md:w-[40%] mt-5 flex flex-col">
            ...
            <div className="flex flex-col p-5 gap-5 my-14 h-full justify-between">
              ...
              <div className="flex flex-col">
                ...
                <button className="bg-green-700 hover:bg-yellow-300 rounded-lg text-white transition-all ease-linear duration-300 border-2 border-green-600 font-bold hover:text-green-700 p-3 text-xl mt-5">
                  {" "}
                  Checkout Now{" "}
                </button>{" "}
              </div>{" "}
